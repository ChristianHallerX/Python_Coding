...
def twoSumII(numbers: list[int], target: int) -> list[int]:
    ''' left/right pointer solution to go over list only once -> time complecity O(n)
    ...
    while left_pointer < right_pointer:
        ...
        # if sum bigger than target, shift right pointer one left
        if current_sum > target:
            right_pointer -= 1
        # if sum smaller than target, shift left pointer one to right

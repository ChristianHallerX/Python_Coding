...
def groupAnagramsZeroArray(strs):
    ...
    for string in strs:
        ...
        for char in string:
            count[ord(char) - ord('a')] += 1  # write to list at index by using char's ASCII value a->0, b->1
        hash_map[tuple(count)].append(string)  # write the count list/tuple as key and the string as value to hash_map
...
def groupAnagramsSorted(strs):
    ...
    for string in strs:
        ...
        if sortedString not in hash_map:
            hash_map[sortedString] = [string]  # write sorted string (list) as key and string as value to hash_map
        else:
            hash_map[sortedString].append(string)  # append the list with additional strings with same sortedString

    return list(hash_map.values())  # return all values as one list of lists


def main():
    print(groupAnagramsZeroArray(strs=["eat", "tea", "tan", "ate", "nat", "bat"]), 'expected: [["bat"],["nat", "tan"],\
    ["ate", "eat", "tea"]]')
    print(groupAnagramsZeroArray(strs=[""]), 'expected: [[""]]')
    print(groupAnagramsZeroArray(strs=["a"]), 'expected: [["a"]]')

    print(groupAnagramsSorted(strs=["eat", "tea", "tan", "ate", "nat", "bat"]), 'expected: [["bat"],["nat", "tan"],\
    ["ate", "eat", "tea"]]')
